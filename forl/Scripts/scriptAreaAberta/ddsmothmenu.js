...
//Specify full URL to down and right arrow images (23 is padding-right for top level LIs with drop downs, 6 is for vertical top level items with fly outs):
    arrowimages: { down: ['downarrowclass', '../../Content/cssAreaAberta/imagens/down.png', 23], right: ['rightarrowclass', '../../Content/cssAreaAberta/imagens/right.gif', 6] },
...
buildmenu: function($, setting){
	...
	var iOS = ( navigator.userAgent.match(/(iPad|iPhone|iPod|Android)/g) ? true : false );
         var method;// = smoothmenu.detecttouch? 'toggle' : setting.method === 'toggle'? 'toggle' : 'hover';
         if(iOS){
                   method = smoothmenu.detecttouch? 'toggle' : setting.method === 'toggle'? 'toggle' : 'hover';
         }
         else {
                   method = smoothmenu.detecttouch? 'hover' : setting.method === 'hover'? 'toggle' : 'hover';
         }
